// ignore_for_file: depend_on_referenced_packages

export 'navigation/app_routing.dart';
export 'navigation/route_name.dart';

export 'theme/app_theme.dart';
export 'theme/bloc/theme_bloc.dart';
export 'theme/bloc/theme_event.dart';
export 'theme/bloc/theme_state.dart';
export 'theme/themes/theme_persistence.dart';

export 'language/language_persistence.dart';

export '../l10n/app_localizations.dart';

export 'package:flutter_localizations/flutter_localizations.dart';
export 'package:expense_mate/core/providers/app_providers.dart';

export 'package:flutter/material.dart';
export 'package:flutter_bloc/flutter_bloc.dart';
export 'package:expense_mate/core/language/bloc/language_bloc.dart';
export 'package:expense_mate/core/language/bloc/language_state.dart';
export 'package:hive/hive.dart';

export 'package:expense_mate/core/navigation/bloc/nav_bloc.dart';
export 'package:expense_mate/core/navigation/bloc/nav_event.dart';
export 'package:expense_mate/core/navigation/bloc/nav_state.dart';
export 'package:expense_mate/features/analytics/presentation/faces/analytics_face.dart';

export 'package:expense_mate/features/profile/presentation/faces/profile_face.dart';
export 'package:expense_mate/features/records/presentation/faces/records_face.dart';
export 'package:expense_mate/core/constant/app_constant.dart';
export 'package:expense_mate/core/language/bloc/language_event.dart';
export 'package:expense_mate/features/profile/presentation/widgets/template_preview.dart';
export 'package:expense_mate/features/profile/presentation/faces/language_face.dart';
export 'package:expense_mate/features/profile/presentation/faces/template_face.dart';
export 'package:expense_mate/core/data/models/category_hive_model.dart';
export 'package:expense_mate/core/utils/translation_helper.dart';
export 'package:expense_mate/features/transcation/presentation/bloc/transcation_bloc/transcation_bloc.dart';
export 'package:expense_mate/features/transcation/presentation/bloc/transcation_bloc/transcation_state.dart';
export 'package:expense_mate/features/transcation/presentation/bloc/transcation_bloc/transcation_event.dart';

export 'package:expense_mate/features/transcation/presentation/components/category_section/category_grid.dart';
export 'package:expense_mate/features/transcation/presentation/components/category_section/mode_button.dart';
export 'package:expense_mate/features/auth/presentation/bloc/auth_bloc.dart';
export 'package:expense_mate/features/auth/presentation/bloc/auth_event.dart';
export 'package:expense_mate/features/auth/presentation/bloc/auth_state.dart';
export 'package:expense_mate/features/auth/presentation/component/login_component.dart';
export 'package:expense_mate/features/auth/presentation/component/quick_login_component.dart';
export 'package:expense_mate/features/auth/presentation/component/toggle_button_component.dart';
export 'package:expense_mate/features/auth/presentation/widgets/auth_logo.dart';
export 'package:expense_mate/features/auth/presentation/widgets/login_flip_card.dart';
export 'package:expense_mate/features/auth/presentation/widgets/login_header.dart';
export 'package:expense_mate/core/data/models/currency_model.dart';
export 'package:expense_mate/features/auth/presentation/widgets/custom_button.dart';
export 'package:expense_mate/features/auth/presentation/widgets/custom_text_field.dart';
export 'package:expense_mate/features/auth/domain/use_cases/check_auth_status_usecase.dart';
export 'package:expense_mate/features/auth/domain/use_cases/get_all_accounts_usecase.dart';
export 'package:expense_mate/features/auth/domain/use_cases/login_with_email_usecase.dart';
export 'package:expense_mate/features/auth/domain/use_cases/login_with_pin_usecase.dart';
export 'package:expense_mate/features/auth/domain/use_cases/logout_usecase.dart';
export 'package:expense_mate/features/auth/domain/use_cases/register_usecase.dart';
export 'package:expense_mate/features/auth/domain/repository/auth_repository.dart';
export 'package:expense_mate/core/data/repository_imp/category_repository_imp.dart';
export 'package:expense_mate/features/auth/data/data_source/auth_data_source.dart';
export 'package:expense_mate/features/auth/data/data_source/currency_data_source.dart';
export 'package:expense_mate/features/auth/data/repositories/auth_repository_imp.dart';
export 'package:expense_mate/features/auth/data/repositories/currency_repository_imp.dart';
export 'package:expense_mate/features/auth/domain/use_cases/change_password_usecase.dart';
export 'package:expense_mate/core/services/hive_initializer.dart';
export 'package:expense_mate/core/data/models/user_model.dart';

// export 'package:expense_mate/navigation_fram.dart';
