...
class MainFrame extends StatelessWidget {
  ...
  Widget _buildFAB(BuildContext context, int currentIndex) {
    final t = AppLocalizations.of(context)!;
    // Change FAB based on current tab
    if (currentIndex == 1) {
      // Budgets tab - show "Create Budget" FAB
      return FloatingActionButton.extended(
        elevation: 10,
        backgroundColor: Theme.of(context).primaryColor,
        onPressed: () => _showCreateBudgetBottomSheet(context),
        icon: const Icon(Icons.add, size: 24),
        label: Text(t.budgets, style: TextStyle(fontWeight: FontWeight.w600)),
      );
    }
